...
import { ScrollHeading, ScrollParagraph, ScrollCard } from './ScrollReveal';

export const ExpertiseSectionPrototype: React.FC = () => {
  const { t } = useLanguage();
  ...
  return (
    <section id="expertise" className="w-full bg-[#0b1229] py-16 px-5 md:px-10 lg:px-20 relative overflow-hidden">
      <div className="max-w-7xl mx-auto flex flex-col gap-10">
        ...
        {/* 3 Column Grid with Staggered Pushed Cards (0.94 → 1.0) */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
          {categories.map((category, index) => {
            const isSecondary = category.accentColor === 'secondary';

            return (
              <ScrollCard key={category.id} index={index}>
                <div className="h-full p-6 sm:p-8 bg-surface-container-low rounded-2xl border border-primary/15 flex flex-col gap-6 shadow-[0_8px_30px_rgba(0,0,0,0.5)] hover:border-secondary/30 transition-all group">
                  {/* Category Header */}
